...
for (i in 1:N.sets) {
  ...
  if(optimal.calinsky == 3){
    ...
    side.labels = c(paste0("HML: ", gsub(","," /",toString(count(annotation, "HML_cluster")$freq[c(2,3,1)]))))
  ...
  if(optimal.calinsky == 4){
    ...
    side.labels = c(paste0("k4: ", gsub(","," /",toString(count(annotation, "ICR_cluster_k4")$freq))),
                    paste0("HML: ", gsub(","," /",toString(count(annotation, "HML_cluster")$freq[c(2,3,1)]))))
  ...
  if(optimal.calinsky == 5){
    ...
    side.labels = c(paste0("k5: ", gsub(","," /",toString(count(annotation, "ICR_cluster_k5")$freq))),
                    paste0("HML: ", gsub(","," /",toString(count(annotation, "HML_cluster")$freq[c(2,3,1)]))))
  ...
  ## plot heatmap
  png(paste0("./5_Figures/Heatmaps/ICR_Heatmaps/", download.method, "/3.4.ICR_Heatmap.3_RNASeq_",Cancer,"_final.png"),res=600,height=9.5,width=11,unit="in")
  ...
  title(main = list(paste0(Cancer, ": RNASeq", "\n N patients = ", nrow(table_cluster_assignment)),cex = 1.7),
        outer = FALSE, line = -4 , adj = 0.48)
  title(sub = list(paste0("Figure: Gene expression data was obtained from TCGA, pancancer normalized (EDAseq) \n and log transformed",
                          ". Samples are ordered by ICR score. ", HML_actions), cex = 1), outer = FALSE, line = 0, adj = 0.40)
