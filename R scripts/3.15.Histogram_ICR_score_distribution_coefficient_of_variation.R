...
required.packages = c("raster", "ggplot2")
...
results = data.frame(Cancer = CancerTYPES[-which(CancerTYPES %in% c("LAML", "DLBC"))], Coefficient_of_variation = NA,
                     Mean_ICR = NA)
...
for (i in 1:N.sets) {
  ...
  cv = cv(x = table_cluster_assignment$ICRscore)
  results$Coefficient_of_variation[which(results$Cancer == Cancer)] = cv
  mean = mean(table_cluster_assignment$ICRscore)
  results$Mean_ICR[which(results$Cancer == Cancer)] = mean
  
  png(paste0("./5_Figures/ICR_distribution_plots/ICR_score_histograms/", download.method, "/v3_Histogram_", Score_type , "_",Cancer,".png"), res=600,height= 1.9,width= 1.9,unit="in")
  ...
  histogram_ICR = qplot(table_cluster_assignment$ICRscore,
                        bins = 60,
                        main = paste0(Cancer, " (cv = ", round(cv, 0), ")"),
                        xlab = Score_type,
                        ylab = "frequency",
                        fill = I("#EB6D7D"),
                        col = I("black"),
...
dir.create("~/Dropbox (TBI-Lab)/DB-LAB/Manuscripts/Pancancer Jessica/For Submission JITC/Revision/New_results", showWarnings = FALSE)
dir.create("~/Dropbox (TBI-Lab)/DB-LAB/Manuscripts/Pancancer Jessica/For Submission JITC/Revision/New_results/Variation", showWarnings = FALSE)
write.csv(results, file = "~/Dropbox (TBI-Lab)/DB-LAB/Manuscripts/Pancancer Jessica/For Submission JITC/Revision/New_results/Variation/cv_results.csv")
