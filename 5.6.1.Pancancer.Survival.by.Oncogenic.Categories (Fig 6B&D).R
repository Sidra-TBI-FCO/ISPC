...
Pathway_subset = "Pathway_High"
Source_surv_data = "Cell_paper"
version = "5.10."                      # "5.3." (Pancancer) or "3.17." (Per cancer)
...
if (Pathways == "ALL"){
  Pathways = names(Selected.pathways)
}
...
dir.create(paste0("./1_Log_Files/", download.method ,"/5.6.1.Pancancer_Survival_Analysis/"), showWarnings = FALSE)
...
if(Source_surv_data == "Cell_paper"){
  ICR_survival_data = read.csv("./2_Data/TCGA cell 2018 clinical/TCGA_CLINICAL_DATA_CELL_2018_S1.csv", stringsAsFactors = FALSE)
...
if(ICR_subset == "ICR_neutral"){
  Survival_data1 = ICR_survival_data[which(ICR_survival_data$bcr_patient_barcode %in% substring(ICR_neutral_cancer_samples, 1, 12)),]
}
if(ICR_subset == Type_of_Cancer){
  Specific_cancer_type_samples = rownames(Hallmark_and_ICR_cluster_assignment_allcancers)[which(Hallmark_and_ICR_cluster_assignment_allcancers$Cancer == Type_of_Cancer)]
  Survival_data1 = ICR_survival_data[which(ICR_survival_data$bcr_patient_barcode %in% substring(Specific_cancer_type_samples, 1, 12)),]
}

All_survival_analysis_data_ONCO = data.frame(Oncogenic_Pathway = Pathways, p_value = 0, HR=0 , CI_lower=0, CI_upper = 0)
...
for (i in 1:N.pathways){
  Pathway = Pathways[i]
  if(version == "3.17."){
    Pathway_name = paste(Pathway, "_cluster", sep = "")
  }
  if(version == "5.3."){
    Pathway_name = paste(Pathway, "_cluster_Pancancer", sep = "")
  }
  if(Pathway_subset == "Pathway_Low"){
    samples_to_include = rownames(Hallmark_and_ICR_cluster_assignment_allcancers)[which(Hallmark_and_ICR_cluster_assignment_allcancers[,Pathway_name] == 
                                                                                          paste0(Pathway, " Low"))]
  ...
  if(Pathway_subset == "Pathway_High"){
    samples_to_include = rownames(Hallmark_and_ICR_cluster_assignment_allcancers)[which(Hallmark_and_ICR_cluster_assignment_allcancers[,Pathway_name] == 
                                                                                          paste0(Pathway, " High"))]
    patients_to_include = substring(samples_to_include, 1, 12)
    Survival_data = Survival_data1[which(Survival_data1$bcr_patient_barcode %in% patients_to_include),]
  }
  if(nrow(Survival_data) == 0){next}
  ...
  dir.create(paste0("./5_Figures/Pancancer_plots/", download.method, "/Survival_Plots_Oncogenic_Categories_", version), showWarnings = FALSE)
  dir.create(paste0("./5_Figures/Pancancer_plots/", download.method, "/Survival_Plots_Oncogenic_Categories_", version, "/", gsub("/", "_", Pathway_name)), showWarnings = FALSE)
  ...
  All_survival_analysis_data_ONCO[All_survival_analysis_data_ONCO == Pathway, "p_value"] = p[2]
  All_survival_analysis_data_ONCO[All_survival_analysis_data_ONCO == Pathway, "HR"] = signif(exp(mHR.extract@coef),3)[2]
  All_survival_analysis_data_ONCO[All_survival_analysis_data_ONCO == Pathway, "CI_lower"] = CI[2,1]
  All_survival_analysis_data_ONCO[All_survival_analysis_data_ONCO == Pathway, "CI_upper"] = CI[2,2]
  ...
  # plots
  png(paste0("./5_Figures/Pancancer_plots/", download.method, "/Survival_Plots_Oncogenic_Categories_", version, "/", gsub("/", "_", Pathway_name),
             "/5.6.1.Kaplan_Meier_ICR_clusters_", gsub("/", "_", Pathway_name) ,"_",Pathway_subset, "_in_", ICR_subset, "_Pancancer.png"),res=600,height=6,width=8,unit="in")                                                                                           # set filename
  ggkm(mfit,
       timeby=12,
       ystratalabs = levels(TS.Surv[,"Group"]),
       ystrataname = NULL,
       main= paste0("Survival curve across ICR groups in all ", ICR_subset, " and ", Pathway_subset," samples"),
...
save(All_survival_analysis_data_ONCO, file = paste0("./4_Analysis/", download.method, "/Pan_Cancer/Survival_Analysis/5.6.1.Classification_v_", version ,"_", ICR_subset, "_", Pathway_subset,
                                                    "_Survival_analysis_High_vs_Low_Groups_Oncogenic_pathways.Rdata"))
