...
ggkm <- function(sfit,
  ...
  p <- ggplot( .df, aes(time, surv)) +
    ...
    theme_bw() +
    theme(axis.title.x = element_text(vjust = 0.5, size = 9, face = "bold", colour = "black"), axis.text = element_text(size = 11, colour = "black"),  # x-axis and y-axis label font size
          axis.title.y = element_text(size = 9, face = "bold", colour = "black")) +    # y-axis label font size
    scale_x_continuous(xlabs, breaks = times, limits = xlims) +
    scale_y_continuous(ylabs, limits = ylims) +
    theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank()) +
    #theme(legend.position = "none") +
    # MOVE LEGEND HERE BELOW [first is x dim, second is y dim]
    theme(legend.position = c(ifelse(m < 8, .14, .20),ifelse(d < 4, .18, .20))) +
    theme(legend.key = element_rect(colour = NA), legend.text = element_text(size = 11), legend.title = element_blank()) +
  ...
  if(table) {
    ...
    data.table <- ggplot(risk.data,aes(x = time, y = strata, label = format(n.risk, nsmall = 0))) +
      geom_text(size = 3) + theme_bw() +                                                           # tabel font size
      scale_y_discrete(breaks = as.character(levels(risk.data$strata)),
                       labels = rev(ystratalabs)) +
      scale_x_continuous("Numbers at risk", limits = xlims) +
      theme(axis.title.x = element_text(size = 9, vjust = 1, colour = "black"),
    ...
    grid.arrange(p, blank.pic, clip = FALSE, nrow = 2,
                 ncol = 1, heights = unit(c(2, 0),c("null", "null")))
    
    if(returns) {
      a <- arrangeGrob(p, blank.pic, clip = FALSE, nrow = 2,
                       ncol = 1, heights = unit(c(2, 0), c("null", "null")))
      return(a)
    }#if
  } else {
